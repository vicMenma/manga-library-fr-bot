...
from manga_fr_bot.models import ChapterPages, MangaSummary
...
class MangaLibraryBot:
    ...
    async def start_cmd(self, _client: Client, msg: Message) -> None:
        text = (
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "Envoie-moi le nom d'un manga et je chercherai les chapitres francais.\n\n"
            "Commandes utiles:\n"
            "• <code>/library</code> - favoris et reprise de lecture\n"
            "• <code>/latest</code> - futur flux des derniers chapitres FR\n"
        )
    ...
    async def show_library(self, target: Message, user_id: int) -> None:
        ...
        if progress:
            rows.append(
                [InlineKeyboardButton("▶ Continuer", callback_data=f"mfr|continue|{progress[0].manga_id}")]
            )
        for manga_id, title in favorites[:8]:
            rows.append([InlineKeyboardButton(f"⭐ {title[:50]}", callback_data=f"mfr|detail|{manga_id}")])

        if not rows:
            await target.reply(
                "Ta bibliotheque est encore vide.\n\nAjoute des favoris en ouvrant un manga puis en appuyant sur ⭐.",
    ...
    async def latest_cmd(self, _client: Client, msg: Message) -> None:
        await msg.reply(
            "🛠 <b>Latest FR</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\nCette vue arrive ensuite. Pour l'instant, commence par une recherche.",
            reply_markup=self._home_kb(),
        )
    ...
    async def search_handler(self, _client: Client, msg: Message) -> None:
        ...
        await wait.edit(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(manga.title[:56], callback_data=f"mfr|detail|{manga.id}")] for manga in results]
                + [[InlineKeyboardButton("📚 Ma bibliotheque", callback_data="mfr|library|0")]]
            ),
        )
        ...
        *,
        ...
        keyboard = InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("▶ Continuer", callback_data=f"mfr|continue|{manga_id}")]
                if progress
                else [],
            ]
        )
    ...
    async def show_reader(
        ...
        *,
        ...
        caption = (
            f"📖 <b>{manga.title}</b>\n"
            f"{pages.chapter} • Page <b>{page_idx + 1}/{len(pages.page_urls)}</b>"
        )
        rows = [[]]
        if page_idx > 0:
            rows[0].append(
                InlineKeyboardButton("⬅ Page", callback_data=f"mfr|page|{chapter_id}|{manga_id}|{page_idx - 1}")
            )
